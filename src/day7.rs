...
pub fn main() {
    let input = "dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
xuabbxdwkutpsogcfea[tgetfqpgstsxrokcemk]cbftstsldgcqbxf[vwjejomptmifhdulc]ejeroshnazbwjjzofbe";
    ...
    let ips = input.lines().map(|l| {
        ...
        loop {
            ...
            outside.push(line.drain(..offset).collect());
            ...
            let offset = line.find(']').unwrap_or(line.len());
            inside.push(line.drain(..offset).collect());
            line.remove(0);
        }
    ...
    let mut count2 = 0;
    for ip in ips {
        if !find_abba(&ip.inside) && find_abba(&ip.outside) {
            count += 1;
        }

        let out_abas = find_abas(&ip.outside);
        let in_babs = find_abas(&ip.inside);

        'outer: for aba in &out_abas {
            let mut aba = aba.chars();
            let aba0 = aba.next();
            let aba1 = aba.next();

            for bab in &in_babs {
                let mut bab = bab.chars();
...
fn find_abba(list: &[String]) -> bool {
    for s in list {
        let chars: Vec<char> = s.chars().collect();
        let chunks = chars.windows(4);

        for chunk in chunks {
            if chunk.len() == 4 &&
                chunk[0] != chunk[1] &&
                chunk.iter().eq(chunk.iter().rev()) {
                println!("{:?}", list);
                println!("{:?}", chunk);
                return true;
            }
...
fn find_abas(list: &[String]) -> Vec<String> {
    let mut abas = Vec::new();

    for s in list {
        let chars: Vec<char> = s.chars().collect();
        let chunks = chars.windows(3);

        for chunk in chunks {
            if chunk.len() == 3 &&
                chunk[0] != chunk[1] &&
                chunk[0] == chunk[2] {
                abas.push(chunk.iter().cloned().collect());
            }
        }
    }
