...
fn decompressed_len_impl<'a,F>(s: &'a str, f: F) -> usize 
    ...
    let mut rest = s;
    while rest.len() > 0 {
        rest = match find_marker(rest) {
            Some((rpt, interior, de_rest)) => {
                de_len += f(interior)*rpt;
                de_rest
            },
            None => {
                de_len += rest.len();
...
fn find_marker(s: &str) -> Option<(usize,&str,&str)> {
    lazy_static! {
        static ref regex: Regex = Regex::new(r"(?x)
            (?P<pre>.*?)                    # Chars before marker
            (?P<mrkr>\(                     # Opening paren
                (?P<cnt>\d*)x(?P<rpt>\d*)   # 1x2
            \))|                            # Closing paren
            (.*)                            # No markers, match text
        ").unwrap();
    }

    let caps = regex.captures(s).expect(s);
    let match_len = caps[0].len();

    if let Some(mrkr) = caps.name("mrkr") {
        let (cnt, rpt): (usize,usize) = (caps["cnt"].parse().unwrap(), caps["rpt"].parse().unwrap());

        Some((rpt, &s[match_len..match_len+cnt], &s[match_len+cnt..]))
    } else {
        None
    }
}