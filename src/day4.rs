...
jyfvnlupj-ibuuf-svnpzapjz-851[gmsnf]";
    ...
    let rooms = input.lines()
        .map(|l| {
            let cap = regex_line.captures_iter(l).next().unwrap();
            
            let sector_id = cap.at(2).unwrap().parse().unwrap();
            let name = cap.at(1).unwrap().to_string();
            let name_decrypted: String = name.replace("-", " ")
                .chars()
                .map(|c| rot(c, sector_id))
                .collect();

            if name_decrypted.contains("northpole") {
                println!("{:?} - {}", name_decrypted, sector_id);
            }

            Room {
                name: name,
                name_decrypted: name_decrypted,
                sector_id: sector_id,
                checksum: cap.at(3).unwrap().to_string()
            }
        });
    
    let real_rooms = rooms
        .filter(|room| {
            let namec = room.name.chars();
            let mut occur = HashMap::new();
            for c in namec.filter(|&c| c != '-') {
                *occur.entry(c).or_insert(0) += 1;
            }

            let mut occur_vec: Vec<_> = occur.iter().collect();
            occur_vec.sort_by_key(|&(a,_)| a);
            occur_vec.sort_by_key(|&(_,b)| -b);

            let actual_chksum: String = (&occur_vec[0..5]).iter()
                .map(|x| *x.0).collect();

            room.checksum == actual_chksum
        });

    let sum = real_rooms.fold(0, |sum, room| sum + room.sector_id);
...
struct Room {
    name: String,
    name_decrypted: String,
    sector_id: i32,
    checksum: String,
}

fn rot(c: char, r: i32) -> char {
    let base = match c {
        'a'...'z' => 'a' as u8,
        'A'...'Z' => 'A' as u8,
        _ => return c
